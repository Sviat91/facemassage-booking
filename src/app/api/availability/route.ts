...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!from || !until) {
      log.warn({ from, until }, 'Availability request missing required dates')
    ...
    log.debug({ from, until, procedureId, debug, minDuration }, 'Computing availability window')

    const data = await getAvailableDays(from, until, minDuration, { debug })
    return NextResponse.json(data)
  } catch (err: any) {
    log.error({ err, from, until, procedureId, debug }, 'Failed to compute availability')
    await reportError(err, {
      tags: { module: 'api.availability' },
      extras: { from, until, procedureId, debug },
    })
